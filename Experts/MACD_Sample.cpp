   ...
   if(total<1)
     {
      ...
      if(MacdCurrent<0 && MacdCurrent>SignalCurrent && MacdPrevious<SignalPrevious && 
        ...
        {
         ticket=OrderSend(Symbol(),OP_BUY,Lots,Ask,3,0,Ask+TakeProfit*_Point,"macd sample",16384,0,Green);
         if(ticket>0)
           {
      ...
      if(MacdCurrent>0 && MacdCurrent<SignalCurrent && MacdPrevious>SignalPrevious && 
        ...
        {
         ticket=OrderSend(Symbol(),OP_SELL,Lots,Bid,3,0,Bid-TakeProfit*_Point,"macd sample",16384,0,Red);
         if(ticket>0)
           {
            if(OrderSelect(ticket,SELECT_BY_TICKET,MODE_TRADES))
                  ...
                  if(OrderStopLoss()<Bid-_Point*TrailingStop)
                    {
                     ...
                     if(!OrderModify(OrderTicket(),OrderOpenPrice(),Bid-_Point*TrailingStop,OrderTakeProfit(),0,Green))
            ...
            if(TrailingStop>0)
              {
               if((OrderOpenPrice()-Ask)>(_Point*TrailingStop))
                 {
                  if((OrderStopLoss()>(Ask+_Point*TrailingStop)) || (OrderStopLoss()==0))
                    {
                     //--- modify order and exit
                     if(!OrderModify(OrderTicket(),OrderOpenPrice(),Ask+_Point*TrailingStop,OrderTakeProfit(),0,Red))
                        Print("OrderModify error ",GetLastError());
                     return;
                    }
                 }
