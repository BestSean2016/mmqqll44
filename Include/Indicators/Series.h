...
//+------------------------------------------------------------------+
//--OLD-- void CSeries::SetSymbolPeriod(const string symbol,const int period)
void CSeries::SetSymbolPeriod(const string& symbol,const int period)
  {
//--OLD--    m_symbol=(symbol==NULL) ? Symbol() : symbol;
       m_symbol=(symbol.empty()) ? Symbol() : symbol;
   m_period=(period==0)    ? Period() : period;
   PeriodToTimeframeFlag((MQL4::ENUM_TIMEFRAMES)m_period);
  }
//+------------------------------------------------------------------+
